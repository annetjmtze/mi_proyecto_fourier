...
if uploaded_file is not None:
    ...
    # Manejo robusto de números con coma decimal
    ...
    muestras_centradas = muestras - media_original
    ...
    with st.spinner("Calculando componentes de Fourier..."):
        for f in frecuencias:
            ...
            a_n = (2 / N) * np.sum(muestras_centradas * cos_wave)
            b_n = (2 / N) * np.sum(muestras_centradas * sin_wave)
            
            # Magnitud, ángulo y fase
            mag = np.sqrt(a_n**2 + b_n**2)
            ang = np.degrees(np.arctan2(b_n, a_n))  # en grados
            fase = -ang  # según definición común
    ...
    reconstruida = np.full_like(t, media_original)
    ...
    def get_table_export(df_to_export, format_type='png'):
        # Formatear para la exportación visual
